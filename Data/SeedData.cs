...
namespace HealthWellbeing.Data
{
    public static class SeedData
    {
        public static void Populate(HealthWellbeingDbContext db)
        {
            ...
            // ✅ 1º Clientes → 2º UtentesSaude (1–1) → 3º Consultas
            PopulateClients(db);
        ...
        // ---------------------------------------------------------------------
        // UTENTES SEED (fonte única: daqui criamos Clients + UtenteSaude 1–1)
        // ---------------------------------------------------------------------
        private static UtenteSeed[] GetUtentesSeed() => new[]
        {
            new UtenteSeed
            {
        ...
        private static void PopulateClients(HealthWellbeingDbContext db)
        {
            ...
            var seeds = GetUtentesSeed();

            var clients = seeds.Select(u => new Client
            {
                Name = u.NomeCompleto,
                Email = u.Email,
                Phone = u.Telefone,
                ...
                // não tens género no seed
                Gender = "",

                // restantes campos do model Client (defaults)
        ...
        // ---------------------------------------------------------------------
        // UTENTE SAÚDE (SEM geração automática; 1–1 com Client via Email)
        // ---------------------------------------------------------------------
        private static void PopulateUtenteSaude(HealthWellbeingDbContext db)
        {
            ...
            {
                if (!clientsByEmail.TryGetValue(s.Email, out var client))
                    throw new InvalidOperationException($"Não foi encontrado Client para o email: {s.Email}");

                // ✅ 1-1: um UtenteSaude por Client
        ...
        // ---------------------------------------------------------------------
        // CONSULTAS (com IdMedico, IdEspecialidade, IdUtenteSaude)
        // ---------------------------------------------------------------------
        ...
        private static void PopulateNutritionists(HealthWellbeingDbContext db)
        {
            ...
            {
                new Nutritionist { Name = "Daniel Rocha",   Gender = "Male",   Email = "daniel.rocha@healthwellbeing.pt" },
                new Nutritionist { Name = "Eduarda Nogueira", Gender = "Female", Email = "eduarda.nogueira@healthwellbeing.pt" },
                new Nutritionist { Name = "Fábio Gonçalves", Gender = "Male",   Email = "fabio.goncalves@healthwellbeing.pt" }
            };
